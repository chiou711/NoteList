...
package com.cw.ListNote.note_add;
...
public class Add_note_option {
     ...
     *
     */
    static List<Add_note_option> addListNote;
    ...
    public static void createSelection(AppCompatActivity act, boolean permitted)
    {
        ...
        addListNote = new ArrayList<>();

        // text
        addListNote.add(new Add_note_option(ID_NEW_TEXT,
                android.R.drawable.ic_menu_edit,
                R.string.note_text));

        if(permitted) {
            // drawing
            addListNote.add(new Add_note_option(ID_NEW_DRAWING,
                    R.drawable.ic_menu_draw,
                    R.string.note_drawing));

            // recording
            addListNote.add(new Add_note_option(ID_NEW_RECORDING,
//                    android.R.drawable.ic_btn_speak_now,
                    R.drawable.ic_mic,
                    R.string.note_recording));

            // audio
            addListNote.add(new Add_note_option(ID_NEW_AUDIO,
                    R.drawable.ic_audio_unselected,
                    R.string.note_ready_audio));

            // camera image
            if (packageManager.hasSystemFeature(PackageManager.FEATURE_CAMERA_ANY)) {
                addListNote.add(new Add_note_option(ID_NEW_CAMERA_IMAGE,
                        android.R.drawable.ic_menu_camera,
                        R.string.note_camera_image));
            }

            // ready image
            addListNote.add(new Add_note_option(ID_NEW_READY_IMAGE,
                    android.R.drawable.ic_menu_gallery,
                    R.string.note_ready_image));

            // camera video
            if (packageManager.hasSystemFeature(PackageManager.FEATURE_CAMERA_ANY)) {
                addListNote.add(new Add_note_option(ID_NEW_CAMERA_VIDEO,
                        android.R.drawable.presence_video_online,
                        R.string.note_camera_video));
            }

            // ready video
            addListNote.add(new Add_note_option(ID_NEW_READY_VIDEO,
                    R.drawable.ic_ready_video,
                    R.string.note_ready_video));
        }

        // YouTube link
        addListNote.add(new Add_note_option(ID_NEW_YOUTUBE_LINK,
                android.R.drawable.ic_menu_share,
                R.string.note_youtube_link));

        // Web link
        addListNote.add(new Add_note_option(ID_NEW_WEB_LINK,
                android.R.drawable.ic_menu_share,
                R.string.note_web_link));

        // Back
        addListNote.add(new Add_note_option(ID_NEW_BACK,
                R.drawable.ic_menu_back,
                R.string.btn_Cancel));

        // Setting
        addListNote.add(new Add_note_option(ID_NEW_SETTING,
        ...
        if( (addListNote != null  ) && (addListNote.size() > 0))
        {
        ...
        gridView.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                ...
                startAddNoteOption(act, addListNote.get(position).option_id);
            }
        });
    ...
    static class GridIconAdapter extends BaseAdapter {
        ...
        public View getView(int position, View convertView, ViewGroup parent) {
            ...
            Drawable drawable = act.getResources().getDrawable(addListNote.get(position).option_drawable_id);
            holder.imageView.setImageDrawable(drawable);
            holder.text.setText(addListNote.get(position).option_string_id);
            return view;
        }
